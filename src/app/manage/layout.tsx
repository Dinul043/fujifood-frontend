    ...
    ;(async () => {
      try {
        ...
        if (role === 'restaurant_admin') {
          ...
          // Delivery staff: same orders page, limited actions
          setOk(true)
          setNm(data.name?.[0] || 'D')
          setTenantId(data.tenant_id)
          setIsOwner(false)
          setUserRole(role)
          // Redirect to orders page if on restricted routes
          if (path.startsWith('/manage/business') || path.startsWith('/manage/history') || path.startsWith('/manage/reports') || path.startsWith('/manage/customers')) {
